main.func<-function(Y, Ytype, Y.cov, Z, lod, aft.cov, trans, alpha.method, eta.method){
  ...
  ##  NOTE: this method works for p<5 and only the marginal approach for eta is implemented for p=4
  
   
  pp<-dim(Z)[2]
  
  if(pp>4){ output<-"not allow p>4" }else{
    if( pp==4 && eta.method=="mKM"){
      output <- "mKM is not available for p=4"
    }else{
      if( Ytype=="cts"|| Ytype=="bin" || Ytype=="count"){
        ...
        if(pp==4){
          ...
          data$delta2 <- ifelse(data$z2>=l2, 1, 0)
          data$delta3 <- ifelse(data$z3>=l3, 1, 0)
          data$delta4 <- ifelse(data$z4>=l4, 1, 0)
          
          data$z1st <- pmax(data$z1, l1)
          data$z2st <- pmax(data$z2, l2)
          data$z3st <- pmax(data$z3, l3)
          data$z4st <- pmax(data$z4, l4)
          
          data$Z <- cbind(data$z1st, data$z2st, data$z3st, data$z4st)
          ...
          idx.c2 <- which(data$delta2==0)
          idx.c3 <- which(data$delta3==0)
          idx.c4 <- which(data$delta4==0)
          
          idx.cen <- unique(c(idx.c1, idx.c2, idx.c3, idx.c4))
          
          idx.1111 <<- which(data$delta1*data$delta2*data$delta3*data$delta4==1)
          idx.0111 <<- which((1-data$delta1)*data$delta2*data$delta3*data$delta4==1)
          idx.1011 <<- which((1-data$delta2)*data$delta1*data$delta3*data$delta4==1)
          idx.1101 <<- which((1-data$delta3)*data$delta1*data$delta2*data$delta4==1)
          idx.1110 <<- which((1-data$delta4)*data$delta1*data$delta2*data$delta3==1)
          
          idx.1100 <<- which(data$delta1*data$delta2*(1-data$delta3)*(1-data$delta4)==1)
          idx.1010 <<- which(data$delta1*data$delta3*(1-data$delta2)*(1-data$delta4)==1)
          idx.1001 <<- which(data$delta1*data$delta4*(1-data$delta2)*(1-data$delta3)==1)
          idx.0110 <<- which(data$delta2*data$delta3*(1-data$delta1)*(1-data$delta4)==1)
          idx.0101 <<- which(data$delta2*data$delta4*(1-data$delta1)*(1-data$delta3)==1)
          idx.0011 <<- which(data$delta3*data$delta4*(1-data$delta1)*(1-data$delta2)==1)
          idx.0001 <<- which(data$delta4*(1-data$delta1)*(1-data$delta2)*(1-data$delta3)==1)
          idx.0010 <<- which(data$delta3*(1-data$delta1)*(1-data$delta2)*(1-data$delta4)==1)
          idx.0100 <<- which(data$delta2*(1-data$delta1)*(1-data$delta3)*(1-data$delta4)==1)
          idx.1000 <<- which(data$delta1*(1-data$delta2)*(1-data$delta3)*(1-data$delta4)==1)
          idx.0000 <<- which((1-data$delta1)*(1-data$delta2)*(1-data$delta3)*(1-data$delta4)==1)
          ...
          if(alpha.method=="rank"){
            ...
            alpha2 <- coef.rankAFT(y=T2, x=data.frame(aft.cov),  delta = data$delta2)
            alpha3 <- coef.rankAFT(y=T3, x=data.frame(aft.cov),  delta = data$delta3)
            alpha4 <- coef.rankAFT(y=T4, x=data.frame(aft.cov),  delta = data$delta4)
          }
          
          if(alpha.method=="LS"){
            data$id<-1:n
            alpha.x<-data.frame(t1=exp(T1), t2=exp(T2), t3=exp(T3), t4=exp(T4), delta1=data$delta1, delta2=data$delta2, delta3=data$delta3, delta4=data$delta4, aft.cov, id=data$id)
            long.my<-reshape(alpha.x, idvar="id", direction="long", varying=list(t=c(1,2,3,4), delta=c(5,6,7,8)), v.names = c("expZ", "Delta") )
            long.my$margin<-factor(long.my$time)
            
            fit1.ex <- aftgee(as.formula(form.aft.cov), id = id, margin=margin, data = long.my, corstr = "ex", B=0)$coefficients
            alpha1 <- fit1.ex[c(1,5,9),2]
            alpha2 <- fit1.ex[c(2,6,10),2]
            alpha3 <- fit1.ex[c(3,7,11),2]
            alpha4 <- fit1.ex[c(4,8,12),2]
            alphax.x <- long.my <- fit1.ex <- NULL
          }
          ...
          r2 <- c(T2-alpha2%*%t(cbind(1,aft.cov)))
          r3 <- c(T3-alpha3%*%t(cbind(1,aft.cov)))
          r4 <- c(T4-alpha4%*%t(cbind(1,aft.cov)))
          
          if(eta.method=="marg"){
            S1.all <- survf(r1,data$delta1)
            S2.all <- survf(r2,data$delta2)
            S3.all <- survf(r3,data$delta3)
            S4.all <- survf(r4,data$delta4)
            
            res1<-S1.all$time[S1.all$prob>0]
            res2<-S2.all$time[S2.all$prob>0]
            res3<-S3.all$time[S3.all$prob>0]  
            res4<-S4.all$time[S4.all$prob>0]  
            ...
            f1 <<- c(S1.all$prob[S1.all$prob>0])
            f2 <<- c(S2.all$prob[S2.all$prob>0])
            f3 <<- c(S3.all$prob[S3.all$prob>0])
            f4 <<- c(S4.all$prob[S4.all$prob>0])
            ...
            S4.all<-S3.all<-S1.all<-S2.all<-NULL
          ...
          tmp4 <- matrix(rep(r4, each = n4), n4, n) 
          ord4 <- colSums(ifelse(tmp4>=res4, 1, 0))
          
          ttt <- tmp1 <- tmp2 <- tmp3 <- tmp4 <- NULL
          
          ord.res <<- cbind(ord1,ord2,ord3,ord4)
          
          est.z1.r <<- gf(outer(res1, c(alpha1%*%t(cbind(1,aft.cov))),"+"))
          est.z2.r <<- gf(outer(res2, c(alpha2%*%t(cbind(1,aft.cov))),"+"))
          est.z3.r <<- gf(outer(res3, c(alpha3%*%t(cbind(1,aft.cov))),"+"))
          est.z4.r <<- gf(outer(res4, c(alpha4%*%t(cbind(1,aft.cov))),"+"))
          
          form.z <-"+z1st+z2st+z3st+z4st"
          formula.cc <- as.formula(paste0(form.y.cov, form.z))
          ...
          if(Ytype=="cts"){
            ...
            theta.hat <- try(multiroot(pslogL.LM.marg.p4, theta, parms = list(data=data, sigma.0=sigma.0, p=p, n=n))$root)
          ...
          if(Ytype=="bin"){
            ...
            theta.hat <- try(multiroot(pslogL.BN.marg.p4, theta, parms = list(data=data, p=p, n=n))$root)
          ...
          if(Ytype=="count"){
            ...
            theta.hat <- try(multiroot(pslogL.PO.marg.p4, theta, parms = list(data=data, p=p, n=n))$root)
            
          }
          
          rm("idx.0000","idx.0001","idx.0010","idx.0011","idx.0100",
             "idx.0101","idx.0110","idx.0111","idx.1000","idx.1001",
             "idx.1010","idx.1011","idx.1100","idx.1101","idx.1110",
             "idx.1111", pos = ".GlobalEnv")
          rm("f1","f2","f3","f4","f12","f13","f14","f23","f24","f34", "n1","n2","n3","n4", pos = ".GlobalEnv")
          rm("ord.res","est.z1.r","est.z2.r","est.z3.r","est.z4.r", pos = ".GlobalEnv") 
